...
void radix_step(struct tmp_in_edge * buf_1, struct tmp_in_edge * buf_2,
		u8_t * tmp, u64_t * counts, u64_t num_edges, u64_t rbits, u64_t bits_offset, bool is_src)
{
	...
	for (u64_t j = 0; j < num_edges; j++)
	{
		if(is_src != true)
			...
			radix_value = ((1 << rbits) - 1) & (((*(buf_1+j)).src_vert)>>bits_offset);
		tmp[j] = radix_value;
		u64_t k = radix_value;
		counts[k]++;
	}
...
void radix_sort(struct tmp_in_edge * buf_1, struct tmp_in_edge * buf_2, 
		u64_t num_edges, unsigned int max_vert_id, bool is_src)
{
	...
	while (bit_offset < bits)
	{
		...
		if (flipped)
			radix_step(buf_2, buf_1, tmp, counts, num_edges, rbits, bit_offset, is_src);
		else
			radix_step(buf_1, buf_2, tmp, counts, num_edges, rbits, bit_offset, is_src);
