		...
		bool with_in_edge)
{
	...
	bool sort_needed = false;// judge the file that is right order
	...
	//judge the file that is right order
	while(read_one_edge() != CUSTOM_EOF ){
		...
		else if ( recent_src_vert > src_vert ) {
			sort_needed = true; 
			break;
		}
		else
	...
	//sort the source
	if( sort_needed != true )
		printf( "the file has a right order.\n" );
	else{ 
		printf( "Edge order is not correct, sorting the file...\n" );       
		while ( read_one_edge() != CUSTOM_EOF ){
		...
		tmp_file += "_sorted.txt";
		in = fopen( tmp_file.c_str(), "r" ); 
		if( in == NULL ){ 
			printf( "Cannot open the new sorted file!\n" ); 
			exit(1); 
		} 
	}
    ...
    if(true==sort_needed)
    {
        ...
        tmp_file += origin_edge_file;
        tmp_file += "_sorted.txt";
        std::cout << "delete sorted file "  << tmp_file << std::endl;
        char tmp[1024];
