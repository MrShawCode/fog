//fog_engine_scc is defined for scced queries, such as SSSP.
// characters of fog_engine_scc:
...
#ifndef __FOG_ENGINE_SCC_H__
#define __FOG_ENGINE_SCC_H__
...
class fog_engine_scc{
		static index_vert_array* vert_index;

		static segment_config<VA> *seg_config;
        char * buf_for_write;

        static u32_t scatter_fog_engine_scc_state;
        static u32_t gather_fog_engine_scc_state;
        static u32_t init_fog_engine_scc_state;
	...
	public:
        ...
        int scc_forward_traversal()
        {
            u32_t CONTEXT_PHASE = 0; 
            init_phase(CONTEXT_PHASE, FORWARD_TRAVERSAL, loop_counter);
            if (cal_true_bits_size(CONTEXT_PHASE) == 0)
                return 1;
            u32_t forward_loop = 0;
            //do while loop until convergence
            while(1)
            {
                forward_loop++;
                PRINT_WARNING("In SCC-algorithm forward-converged, SCC_FORWARD_SCATTER and UPDATE loop %d\n", forward_loop);
                CONTEXT_PHASE = (forward_loop%2);
                PRINT_DEBUG("before normal-scatter, num_vert_of_next_phase = %d\n", cal_true_bits_size(1-CONTEXT_PHASE));
                scatter_fog_engine_scc_state = SCC_FORWARD_SCATTER;
                scatter_updates(1-CONTEXT_PHASE, 1);
                cal_threshold();
                PRINT_DEBUG("after normal-scatter and before normal-gather, num_vert_of_next_phase = %d\n", cal_true_bits_size(1-CONTEXT_PHASE));

                gather_fog_engine_scc_state = SCC_FORWARD_GATHER;
                gather_updates(CONTEXT_PHASE, -1);
                //cal_threshold();
            ...
            PRINT_DEBUG("Forward traversal finished\n");
            return 0;
        }
        int scc_backward_traversal()
        {
            u32_t CONTEXT_PHASE = 0; 
            //u32_t ret = 0;
            ...
            init_phase(CONTEXT_PHASE, BACKWARD_TRAVERSAL, loop_counter);
            //print_attr_result();
            //exit(-1);
            if (cal_true_bits_size(CONTEXT_PHASE) == 0)
                return 1;
            u32_t backward_loop = 0;
            //do while loop until convergence
            while(1)
            {
                backward_loop++;
                PRINT_WARNING("In SCC-algorithm backward-converged, SCC_BACKWARD_SCATTER and UPDATE loop %d\n", backward_loop);
                CONTEXT_PHASE = (backward_loop%2);
                PRINT_DEBUG("before normal-scatter, num_vert_of_next_phase = %d\n", cal_true_bits_size(1-CONTEXT_PHASE));
                scatter_fog_engine_scc_state = SCC_BACKWARD_SCATTER;
                scatter_updates(1-CONTEXT_PHASE, 1);
                cal_threshold();
                PRINT_DEBUG("after normal-scatter and before normal-gather, num_vert_of_next_phase = %d\n", cal_true_bits_size(1-CONTEXT_PHASE));

                gather_fog_engine_scc_state = SCC_BACKWARD_GATHER;
                gather_updates(CONTEXT_PHASE, -1);
                //cal_threshold();
                PRINT_DEBUG("after normal-gather, num_vert_of_next_phase = %d\n", cal_true_bits_size(CONTEXT_PHASE));
                if (cal_true_bits_size(CONTEXT_PHASE) == 0)
            ...
            PRINT_DEBUG("BACKward traversal finished\n");
            return 0;
        }
		void operator() ()
		{
            /*
             * operator() for scc algorithm
             * first, do cc algorithm -> convergence, 
             * then, do backward traversal
             */
            int ret = 0;
            while(1)
            {
                loop_counter++;
                PRINT_WARNING("SCC_SCATTER and UPDATE loop %d\n", loop_counter);

                ret = scc_forward_traversal();
                //print_attr_result();
                if (ret == 1)
                    break;
                ret = scc_backward_traversal();
                //print_attr_result();
                //exit(-1);
                if (ret == 1)
                    break;
        ...
        void print_attr_result()
        {
            ...
            if (seg_config->num_attr_buf == 1)
            {
                ...
                for (i = 0; i < 10000; i++)
                    PRINT_DEBUG("attr[%d].prev_root = %d, component_root = %d\n",i, attr_array_head[i].prev_root, attr_array_head[i].component_root);
                for (i = 0; i <= gen_config.max_vert_id; i++)
                    assert(attr_array_head[i].prev_root == attr_array_head[i].component_root);
            }
            else
            {
                ...
                for (i = 0; i < 10000; i++)
                    PRINT_DEBUG("attr[%d].prev_root = %d, component_root = %d\n",i, attr_array_header[i].prev_root, attr_array_header[i].component_root);
                for (i = 0; i <= gen_config.max_vert_id; i++)
                    assert(attr_array_header[i].prev_root == attr_array_header[i].component_root);
		...
		void init_phase(u32_t CONTEXT_PHASE, int SCC_PHASE, u32_t loop_counter)
		{
			...
			for( u32_t i=0; i < seg_config->num_segments; i++ ){
                ...
                if (seg_config->num_segments > 1 && 
                        ((loop_counter > 1) || (loop_counter == 1 && SCC_PHASE == BACKWARD_TRAVERSAL)))
                {
                    io_work * read_io_work = NULL;
                    u32_t offset = 0, read_size = 0;
				...
				if( i != (seg_config->num_segments-1) ){
                    ...
                    p_init_param->PHASE = CONTEXT_PHASE;
                    p_init_param->scc_phase = SCC_PHASE;
                    p_init_param->loop_counter = loop_counter;
					new_cpu_work = new cpu_work<A,VA,U>( init_fog_engine_scc_state, 
						(void*)p_init_param);
        ...
        int scatter_updates(u32_t CONTEXT_PHASE, u32_t global_or_target)
        {
            ...
            if (scatter_fog_engine_scc_state == SCC_FORWARD_SCATTER)
                gather_fog_engine_scc_state = SCC_FORWARD_GATHER;
            if (scatter_fog_engine_scc_state == SCC_BACKWARD_SCATTER)
                gather_fog_engine_scc_state = SCC_BACKWARD_GATHER;
            ...
            if( seg_config->num_attr_buf == 1 ){
                ...
                PRINT_DEBUG("scatter phase:%d\n", phase);
                if (global_or_target == 1)
                    if (ret == 1)
                        PRINT_DEBUG("before context scatter, num_vert_of_next_phase = %d\n", cal_true_bits_size(CONTEXT_PHASE));
                scatter_cpu_work = new cpu_work<A, VA, U>(scatter_fog_engine_scc_state, (void *)p_scatter_param);
                ...
                if (global_or_target == 1 && ret == 1 && unemployed == 1)
                {
                    ...
                    else 
                    {
                        ...
                        for (u32_t k = 0; k < (u32_t)num_unfinished; k++)
                        {
                            ...
                            if (ret_value == 2)
                            {
                                ...
                                /*scatter_cpu_work = new cpu_work<A, VA>(scatter_fog_engine_scc_state, (void *)p_scatter_param);
                                pcpu_threads[0]->work_to_do = scatter_cpu_work;
                                (*pcpu_threads[0])();
                ...
                else if (global_or_target == 0 && ret == 1 && unemployed == 1)
                {
                    assert(num_unfinished > 0);
                    assert((u32_t)num_unfinished < gen_config.num_processors);
                    signal_of_partition_gather = STEAL_GATHER;
            ...
            memset(p_strip_count, 0, sizeof(u32_t) * seg_config->num_segments);
            reset_manager(CONTEXT_PHASE);
            return ret;
        }

        void reset_manager(u32_t CONTEXT_PHASE)
        {
            ...
            for (u32_t i = 0 ; i < gen_config.num_processors; i++ )
            {
                my_context_data = CONTEXT_PHASE > 0 ? seg_config->per_cpu_info_list[i]->target_sched_manager->p_context_data1
                    : seg_config->per_cpu_info_list[i]->target_sched_manager->p_context_data0;
                //if (fog_engine_scc_state == SCC_BACKWARD_SCATTER)
               // {
                    //bitmap * my_bitmap = my_context_data->p_bitmap;
                    //memset(my_bitmap, 0, my_context_data->per_bitmap_buf_size);
                    /*for (u32_t j = my_bitmap->get_start_vert(); j <= my_bitmap->get_term_vert(); j+=gen_config.num_processors)
                    {
                        if (my_bitmap->get_value(j) == 1)
                            PRINT_ERROR("memset is not success!\n");
                    }*/
                    //assert(my_context_data->per_bits_true_size == 0);
                 //   my_context_data->per_bits_true_size = 0;
                  //  sched_list_context_data * my_list_data = seg_config->per_cpu_info_list[i]->global_sched_manager;
                    //assert(my_list_data->num_vert_to_scatter == 0);
                  //  my_list_data->num_vert_to_scatter = my_list_data->normal_sched_vert_to_scatter;
                 //   my_list_data->context_vert_id = 
                    //    my_list_data->context_edge_id = 0;
                  //  my_list_data->context_steal_min_vert = 
                      //  my_list_data->context_steal_max_vert = 0;
                    //my_list_data->context_steal_num_vert = 0;
                  //  my_list_data->signal_to_scatter = 0;
                //}
        ...
        u32_t rebalance_sched_bitmap(u32_t cpu_not_finished_id, u32_t CONTEXT_PHASE)
        {
            ...
            if ((average_num/8) == 0)
            {
                ...
                //    set_signal_to_scatter(CONTEXT_SCATTER, cpu_id, CONTEXT_PHASE);
                return 2;
        ...
        void gather_updates(u32_t CONTEXT_PHASE, int phase)
        {
            ...
            if (seg_config->num_attr_buf == 1)
            {
                ...
                p_gather_param->PHASE = CONTEXT_PHASE;
                p_gather_param->threshold = 0;
                p_gather_param->strip_id = 0;

                gather_cpu_work = new cpu_work<A, VA, U>(gather_fog_engine_scc_state, (void *)p_gather_param);
		...
		void show_sched_bitmap_tasks( sched_bitmap_manager *target_sched_manager )
		{
		}
...
template <typename A, typename VA, typename U>
index_vert_array * fog_engine_scc<A, VA, U>::vert_index;

template <typename A, typename VA, typename U>
u32_t fog_engine_scc<A, VA, U>::scatter_fog_engine_scc_state;

template <typename A, typename VA, typename U>
u32_t fog_engine_scc<A, VA, U>::init_fog_engine_scc_state;

template <typename A, typename VA, typename U>
u32_t fog_engine_scc<A, VA, U>::gather_fog_engine_scc_state;

template <typename A, typename VA, typename U>
u32_t fog_engine_scc<A, VA, U>::current_attr_segment;

template <typename A, typename VA, typename U>
segment_config<VA> * fog_engine_scc<A, VA, U>::seg_config;

template <typename A, typename VA, typename U>
io_queue * fog_engine_scc<A, VA, U>::fog_io_queue;
