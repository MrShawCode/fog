...
template <typename A, typename VA, typename U, typename T>
fog_engine<A, VA, U, T>::~fog_engine()
{
    reclaim_everything();
}
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::print_attr_result()
{
    u32_t i = 0;
    VA * attr_array_head = NULL;
    if (seg_config->num_attr_buf == 1)
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::cal_true_bits_size(u32_t CONTEXT_PHASE)
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::show_all_sched_tasks()
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::init_phase(int global_loop)
{
...
//-1:failure
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::set_signal_to_scatter(u32_t signal, u32_t processor_id, u32_t CONTEXT_PHASE)
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::set_signal_to_gather(u32_t signal, u32_t processor_id, u32_t CONTEXT_PHASE)
{
    sched_bitmap_manager * my_sched_bitmap_manager;
    struct context_data * my_context_data;
    my_sched_bitmap_manager = seg_config->per_cpu_info_list[processor_id]->target_sched_manager;
...
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::scatter_updates(u32_t CONTEXT_PHASE)
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::reset_target_manager(u32_t CONTEXT_PHASE)
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::reset_global_manager(u32_t CONTEXT_PHASE)
{
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::rebalance_sched_bitmap(u32_t cpu_not_finished_id, u32_t CONTEXT_PHASE)
{
...
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::lru_hit_target(int strip_id)
{
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::get_free_buf_num()
{
    if (seg_config->buf0_holder == -1 && seg_config->buf1_holder == -1)
        return 2;
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::get_free_buf_id()
{
    if (seg_config->buf0_holder == -1 && seg_config->buf1_holder != -1)
        return 1;
    else if (seg_config->buf0_holder != -1 && seg_config->buf1_holder == -1)
...
template <typename A, typename VA, typename U, typename T>
char * fog_engine<A, VA, U, T>::get_target_buf_addr(int strip_id)
{
    if (strip_id == -1)
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::do_io_work(int strip_id, u32_t operation, char * io_buf, io_work* one_io_work)
{
    if (one_io_work != NULL)
    {
        fog_io_queue->wait_for_io_task(one_io_work);
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::cal_strip_size(int strip_id, u32_t util_rate_signal, u32_t signal_threshold)
{
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::global_return()
{
    update_map_manager * map_manager;
    u32_t * map_head;
    u32_t total_updates;
...
template <typename A, typename VA, typename U, typename T>
u32_t fog_engine<A, VA, U, T>::cal_threshold()
{
    update_map_manager * map_manager;
    u32_t * map_head;
    u32_t strip_cap;
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::show_update_map(int processor_id, u32_t * map_head)
{
    //print title
...
//-1 means failure
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::map_attr_file()
{
    struct stat st;
    char *memblock;
...
//-1 means failure
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::unmap_attr_file()
{
...
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::map_write_attr_file()
{
    struct stat st;
    char *memblock;
...
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::remap_write_attr_file()
{
    int ret;
    if (attr_fd)
...
//remap
template <typename A, typename VA, typename U, typename T>
int fog_engine<A, VA, U, T>::remap_attr_file()
{
    int ret;
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::add_schedule(u32_t task_vid, u32_t CONTEXT_PHASE)
{
    u32_t  partition_id;
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::reclaim_everything()
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::show_target_sched_update_buf()
{
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::target_init_sched_update_buf()
{
...
// TODO: replace PRINT_DEBUG with PRINT_WARNING or something always show output.
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::show_global_sched_update_buf()
{
    PRINT_DEBUG( "===============\tsched_update buffer for each CPU begin\t=============\n" );
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::global_init_sched_update_buf()
{
    u32_t update_map_size,  total_header_len;
    u64_t strip_buf_size, strip_size;//, aux_update_buf_len;
    u32_t strip_cap;
...
template <typename A, typename VA, typename U, typename T>
void fog_engine<A, VA, U, T>::add_sched_task_to_processor( u32_t processor_id, sched_task *task, u32_t task_len )
{
...
template <typename A, typename VA, typename U, typename T>
void * fog_engine<A, VA, U, T>::map_anon_memory( u64_t size,
                     bool mlocked,
                     bool zero )
